...
# @router.get("/get/{computed_date}/{computed_by_id}/", response_model=ComputedDetailResponse)
# async def read_computed_detail(computed_date: date,computed_by_id: UUID,db: get_db = Depends()):
#     result = ComputedDetailService(db).get_computed_detail(computed_date, computed_by_id)
#     return result


@router.get("/get/", response_model=ComputedDetailResponse)
async def read_computed_detail(
    computed_date: date,
    computed_by_id: UUID,
    db: get_db = Depends()
):
