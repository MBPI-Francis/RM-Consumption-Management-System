			...
			+
                CASE
					WHEN ib.statusname::text = 'held : reject'::text 
						THEN (- COALESCE(rej.total_held, 0::numeric)) 
							- COALESCE(rej.total_released, 0::numeric) 
							+ COALESCE(
					                    CASE
					                        WHEN tf.statusname::text = 'held : reject'::text 
												THEN
													CASE
														WHEN tf.transferred_from_qty > ib.beginningbalance  -- This code is for fixing the computation when UPDATING the warehouse in terms of transfer form
															THEN 0
		
														ELSE
															- tf.transferred_from_qty
													END
					                        ELSE NULL::numeric
					                    END, 0::numeric) 
							+ COALESCE(
					                    CASE
					                        WHEN tt.statusname::text = 'held : reject'::text 
												THEN
													CASE
														WHEN tf.transferred_from_qty > ib.beginningbalance  -- This code is for fixing the computation when UPDATING the warehouse in terms of transfer form
															THEN 0
                    ...
                    WHEN ib.statusname::text = 'held : under evaluation'::text 
						THEN (- COALESCE(eval.total_held, 0::numeric)) 
